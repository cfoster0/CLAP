...
def apply_rotary_pos_emb(x, sincos):
    sin, cos = map(lambda t: repeat(t, "n d -> n (d j)", j=2)[None, :, None, :], sincos)
...
class FixedPositionalEmbedding(nn.Module):
    ...
    def __call__(self, inputs, seq_dim=1):
        ...
        inv_freq = 1.0 / (10e4 ** intervals / dim)
...
class RotaryPositionalEmbedding(FixedPositionalEmbedding):
    ...
    @nn.compact
    def __call__(self, inputs, seq_dim=1):
        sin, cos = super(RotaryPositionalEmbedding, self).__call__(inputs, seq_dim)
